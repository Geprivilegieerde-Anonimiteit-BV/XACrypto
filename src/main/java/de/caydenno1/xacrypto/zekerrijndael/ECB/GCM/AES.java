...
public final class AES implements BlockCipher {
    ...
    private final byte[][] sEnc = new byte[4][4];
    private final byte[][] sDec = new byte[4][4];
    private final byte[] outBuf = new byte[16];

    public AES(byte[] key, int bits) throws XACryptoException {
        this.bits = bits;
        this.rounds = switch (bits) {
            case 128 -> 10;
            case 192 -> 12;
            case 256 -> 14;
    ...
    private static void invSub(byte[][] s) {
        for (int _0 = 0 ; _0 < 4 ; _0++) for (int _1 = 0 ; _1 < 4 ; _1++) s[_0][_1] = (byte) ARIA_XB1 [s[_0][_1] & 0xFF];
    }
    ...
    private void keyExpansion(byte[] key) throws XACryptoException {
        ...
        for (int i = tem; i < kc; i++) {
            System.arraycopy(keys[i - 1], 0, temp, 0, 4);
...
public byte[] encryptCBC(byte[] ln, byte[] iv) throws XACryptoException {
        if (iv == null || iv.length != 16) {
            throw new XACryptoException("CBC IV must be 16 bytes");
        }
