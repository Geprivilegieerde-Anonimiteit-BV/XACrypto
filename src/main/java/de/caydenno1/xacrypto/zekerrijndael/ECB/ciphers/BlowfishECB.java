...
public class BlowfishECB implements ECBExceptionless {
    ...
    public byte[] decrypt(byte[] cip) throws XACryptoException {
        if (cip.length == 0 || cip.length % 8 != 0) throw new XACryptoException("ciptext must be bound to 8-byte alignment");

        byte[] decomp = new byte[cip.length];
        for (int i = 0 ; i < cip.length ; i += 8) engine.decryptBlock(cip, i, decomp, i);

        int pLen = decomp[decomp.length - 1] & 0xFF;
        if (pLen < 1 || pLen > 8) throw new XACryptoException("corrupt data, padding must be 1-8 bytes exclusive", (int)pLen);

        for (int i = decomp.length - pLen ; i < decomp.length ; i++) if (((decomp[i] & 0xFF) != pLen)) throw new XACryptoException("something up with yo padding.. :(");
